...
{
	jQuery(e).parents().each(function(){
		var obj = jQuery(this);
		var classes = obj.attr('class').split(' ');
		if (jQuery.inArray('de77_wysiwyg', classes) > -1)
		{
...
{
	toolbar = jQuery.ajax({url:'jquery.wysiwyg.htm',async:false}).responseText;
	el.parent().prepend(toolbar);

}

(function(jQuery){
 jQuery.fn.de77_wysiwyg = function(options) {
 	if (options == false)
 	{
	...
	else
	{
		return this.each(function() {
			var obj = jQuery(this);
			var classes = obj.attr('class').split(' ');
			if (jQuery.inArray('de77_editor', classes) > -1)
			{
				return false;
			}
